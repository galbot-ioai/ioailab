"""Robot-agnostic observation terms for the PickToShelf pick phase."""
...
def canonical_robot_joint_pos(
    ...
    if missing:
        raise ValueError(f"PickToShelf robot_joint_pos is missing joint(s): {missing}")
...
def make_pick_observations_cfg(
    *,
    joint_names: Sequence[str],
    neutral_joint_names: Sequence[str] = (),
    camera_name: str,
    ...
    """Return the pick phase observation cfg for a robot binding."""

    canonical_joint_names = tuple(str(joint_name) for joint_name in joint_names)
    if not canonical_joint_names:
        raise ValueError("make_pick_observations_cfg requires joint_names.")
    ...
    @configclass
    class PickToShelfPolicyObs(ObsGroup):
        """Last action, canonical joints, and one task camera."""
    ...
    @configclass
    class PickToShelfObservationsCfg:
        """Policy observations for vision-based collection and evaluation."""

        policy: PickToShelfPolicyObs = PickToShelfPolicyObs()

    return PickToShelfObservationsCfg


__all__ = ["canonical_robot_joint_pos", "make_pick_observations_cfg"]
